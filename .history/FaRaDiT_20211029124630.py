...
class Disk():
    ...
    def radmc_write_dust_density(self, binary = True, thickness = f_z, nthet = 100, buffer_size = 4096, note = "", theta_min = np.pi*0.25):
        ...
        if self.density.ndim == 3:
            print(f"Density notice: writing a dust_density."+binary*f"b"+f"inp for radmc3d, a total of {len(self.r_med)}×{nthet}×{len(self.phi_med)} = {np.size(self.density)} cells...")
