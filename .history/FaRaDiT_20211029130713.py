    ...
    def radmc_write_dust_density(self, binary = True, thickness = f_z, nthet = 100, buffer_size = 4096, note = "", theta_min = np.pi*0.25):
        ...
        if self.density.ndim == 3:  #just writing down 3D grid
            ...
            if binary:
                ...
                outfile.write("\n")
                np.transpose(self.density*cm*cm*cm/gram, (2,1,0)).tofile(outfile, sep = "\n", format="%9e") #transposition is required, cuz radmc want its files in such an order: [0,0,0], … [n_rad, 0, 0], [0,1,0], …  [n_rad, 1, 0]. Numpy goes the other way around, first 'depleting' the last index: [0,0,0], … [0, 0, 0], [0,1,0], …  [n_rad, 1, 0]
            outfile.close()
            return 0
