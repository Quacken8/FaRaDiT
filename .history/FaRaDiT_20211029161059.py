...
def main():
    ...
    for iteration in range(10):
        ...
        im = image.readImage()
        image.plotImage(im, au=True, log=True, maxlog=6, saturate=1, cmap=plt.cm.gist_heat, pltshow = False)

        disk.radmc_read_density()
        disk.radmc_read_temperature()
        disk.plot_T_slice(show = False, note = f"Iteration {iteration}", logarithmic_scale=True)
