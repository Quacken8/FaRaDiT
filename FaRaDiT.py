...
def is_file_binary(filename):
    """
    Slow binary checker --- goes character by character till it finds one, that it can't convert to utf-8, which it interprets as it being a binary file symbol
    """
    try:
...
class Disk():
    ...
    def gas_to_dust(self):
        """
        Modifies the disk's density to go from gas to dust, which dominates optical properities (later also according to its temperature to better reflect dust density with its typical evaporations and stuffs)
    ...
    def radmc_write_dust_density(self, binary = True, thickness = f_z, nthet = 100, buffer_size = 4096*32*40, note = ""):
        ...
        if self.density.ndim == 3:  #just writing down 3D grid
            print(f"Density notice: density is in 3D, writing a dust_density."+binary*f"b"+f"inp for radmc3d, a total of {len(self.r_med)}×{nthet}×{len(self.phi_med)} = {np.size(self.density)} cells...")
            nrspecs = 1  # number of dust species
            nrcells = np.size(self.density)
            
            if binary:
                outfile = open("dust_density"+note+".binp","wb")
                np.array([1,8,nrcells,nrspecs]).tofile(outfile)
                np.transpose(self.density*cm*cm*cm/gram, (2,1,0)).tofile(outfile)
            else:
                outfile = open("dust_density"+note+".inp","w")
                np.array([1,nrcells,nrspecs]).tofile(outfile, sep = "\n")
                outfile.write("\n")
                np.transpose(self.density*cm*cm*cm/gram, (2,1,0)).tofile(outfile, sep = "\n", format="%9e") #transposition is required, cuz radmc want its files in such an order: [0,0,0], … [n_rad, 0, 0], [0,1,0], …  [n_rad, 1, 0]. Numpy tofile goes the other way around, first 'depleting' the last index: [0,0,0], … [0, 0, n_phi], [0, 1 ,0], …  [0, 1, n_phi]... I suppose anyway
            outfile.close()
            return 0
    ...
    def radmc_read_density(self, isbinary = None, filename = None):
        ...
        self.density = np.reshape(self.density, (len(self.r_med), len(self.theta_med), len(self.phi_med)))*gram/cm/cm/cm
    ...
    def evaporate_densities(self, evaporation_temp = 300):
        """
        Evaporates silicates by halving its density in each cell that surpases 'evaporation_temp'
        --------------------
        Parameters:
        -------------------- 
        evaporation_temp [K]: double, temperature at which the material should be evaporating
        """
...
def main():
    ...
    for iteration in range(3):
        ...
        im = image.readImage()
        image.plotImage(im, au=True, log=True, maxlog=6, saturate=1, cmap=plt.cm.gist_heat, pltshow = False, fig=fig)

        disk.radmc_read_density()
        disk.radmc_read_temperature()

        disk.plot_rho_slice(show = False, note = f"Iteration {iteration}", logarithmic_scale=True)



        disk.evaporate_densities()

    plt.show()
    input()

    disk.radmc_read_density()
    disk.radmc_write_dust_density(note = "_new", binary = False)
